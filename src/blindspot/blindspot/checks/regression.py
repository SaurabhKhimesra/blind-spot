"""Regression checks. Run these before and after any change.
...
partitioned.py first, because that is where two bugs already hid.
    ...
    ros2 run blindspot regression      (colcon test runs them too)
"""

import numpy as np

from blindspot.study.ibvs_core import (se3_exp, rot_z, rot_x, make_pose, square_target,
                                       transform_points, project, interaction_matrix,
                                       run_ibvs, scenario_camera_retreat, sigma_6)
from blindspot.study.partitioned import run_partitioned, polygon_sigma, line_alpha
from blindspot.study.truncated import run_truncated, pinv_truncated
from blindspot.study.switched import (run_switched, use_partition, calibrate_sigma6,
                                      calibrate_area, calibrate_alpha, calibrate_sigma6_table)
from blindspot.study.partitioned import polygon_sigma as _poly
from blindspot.study.partitioned import XY_COLS
...
from blindspot.study.predict import roll_forward, truth, steps_to_fire   # noqa: E402
...
from blindspot.study.ibvs_core import rot_y as _rot_y  # noqa: E402
from blindspot.study.partitioned import polygon_sigma as _psig  # noqa: E402
...
from blindspot.study.partitioned import line_alpha as _la, wrap as _wrap  # noqa: E402
from blindspot.study.partitioned import Z_COLS as _ZC, XY_COLS as _XC  # noqa: E402
...
# ----------------------------------------------------------------------
# 22. The folding part: the Gazebo arm act, reproduced here in numpy with the
#     ARM'S OWN control law, imported from the demo package rather than
#     transcribed (blindspot/arm_law.py, used by the blindspot_arm package).
#
...
#     drives the camera into the part. sigma_6 never fires, because folding
#     AWAY from the camera adds depth variation and the interaction matrix
#     stays well conditioned. The area guard fires only when the fold outpaces
#     the partition's own depth loop, because the lunge restores the very area
#     the guard watches.
# ----------------------------------------------------------------------
print("\n--- the folding part: the arm act in numpy ---")

from blindspot.arm_law import (DT as _DT, Z_DESIRED as _ZD,  # noqa: E402
                               best_cyclic_match as _bcm,
                               ibvs_twist as _twist,
                               order_by_angle as _oba)

_PHI = np.deg2rad(11.0)      # marker layout rotation used on the panel
_RT, _RS = 0.05, 0.006       # ring radius and marker sphere radius, metres
_FX = (640 / 2) / np.tan(1.0472 / 2)          # the wrist camera in the URDF
_XL, _YL = (640 / 2) / _FX, (480 / 2) / _FX


def _hex22(r):
    a = np.linspace(0, 2 * np.pi, 7)[:-1] + _PHI
...
_CAL22 = dict(pct=1.0, n=4000, seed=0, z_range=(0.18, 0.40), lateral=0.05)
A22 = calibrate_area(_hex22(_RT), **_CAL22)
S22 = calibrate_sigma6(_hex22(_RT), **_CAL22)
check("the demo's guard threshold reproduces from the study's calibrator",
      abs(A22 - 2.009e-01) / 2.009e-01 < 0.02,
      "(%.4e here, 2.009e-01 logged in Gazebo)" % A22)
...
def _fold22(theta):
    ...
    y = P[:, 1].copy()
    P[:, 1] = y * np.cos(theta)
    P[:, 2] = np.abs(y) * np.sin(theta)
    return P


def _act(rule, fold_deg=85.0, fold_s=1.0, t_fold=1.0, T=6.0):
    """Converged at the goal, then the part folds. Free camera, exact dots."""
    cTo = make_pose(t=np.array([0.0, 0.0, _ZD]))
    s_star0 = _oba(_hex22(_RT / _ZD)[:, :2])
    o = dict(standoff=9.0, lost=0, fired=0, first=None, min_area=9.0,
             min_s6=9.0, peak=0.0)
    for k in range(int(T / _DT)):
        t = k * _DT
        P = _fold22(np.deg2rad(fold_deg)
                    * float(np.clip((t - t_fold) / fold_s, 0, 1)))
        Pc = transform_points(cTo, P)
        if np.any(Pc[:, 2] <= 0.02):
            o["lost"] += 1
            break
        s_all = Pc[:, :2] / Pc[:, 2:3]
        vis = (np.abs(s_all[:, 0]) < _XL) & (np.abs(s_all[:, 1]) < _YL)
        rad = _RS / Pc[:, 2]
        merged = any(np.linalg.norm(s_all[i] - s_all[j])
                     < rad[i] + rad[j] + 3 / _FX
                     for i in range(6) for j in range(i + 1, 6))
        v = np.zeros(6)
        if vis.all() and not merged:
            s = _oba(s_all)
            m_area = _poly(s) / A22
            o["min_area"] = min(o["min_area"], m_area)
            o["min_s6"] = min(o["min_s6"],
                              sigma_6(interaction_matrix(s_all, Pc[:, 2])) / S22)
            part = {"partition": True, "ibvs": False,
                    "guard": m_area > 1.0}[rule]
            if rule == "guard" and not part:
                o["fired"] += 1
                if o["first"] is None:
                    o["first"] = round(t - t_fold, 2)
            v = _twist(s, _bcm(s, s_star0), part)
            o["peak"] = max(o["peak"], float(np.linalg.norm(v)))
        else:
            o["lost"] += 1
        cTo = se3_exp(-v * _DT) @ cTo
        o["standoff"] = min(o["standoff"], -float(np.linalg.inv(cTo)[2, 3]))
    return o


_act85 = {r: _act(r) for r in ("partition", "guard", "ibvs")}
check("85 deg fold in 1 s: the partition drives into the part and loses it",
      _act85["partition"]["standoff"] < 0.10 and _act85["partition"]["lost"] > 10,
      "(%.3f m from a 0.26 m standoff, %d steps without all six markers)"
      % (_act85["partition"]["standoff"], _act85["partition"]["lost"]))
check("  the guard fires during the fold and keeps its standoff",
      (_act85["guard"]["first"] is not None
       and _act85["guard"]["first"] <= 1.0
       and _act85["guard"]["standoff"] > 0.15
       and _act85["guard"]["lost"] == 0),
      "(fired %.1f s into the fold, %.3f m)"
      % (_act85["guard"]["first"], _act85["guard"]["standoff"]))
check("  plain IBVS alone holds it too: the partition is what fails",
      _act85["ibvs"]["standoff"] > 0.20 and _act85["ibvs"]["lost"] == 0,
      "(%.3f m)" % _act85["ibvs"]["standoff"])
check("  sigma_6 never fires: folding AWAY adds depth, so L stays conditioned",
      _act85["partition"]["min_s6"] > 1.2,
      "(min sigma_6 margin %.2fx, area fell to %.2fx)"
      % (_act85["partition"]["min_s6"], _act85["partition"]["min_area"]))

# The guard watches a quantity its own controller regulates: when the fold is
# slow enough, the lunge restores the area before it crosses the threshold.
_slow = {r: _act(r, fold_s=3.0, T=9.0) for r in ("partition", "guard")}
check("a SLOW fold is masked: the lunge restores the area, the guard misses",
      _slow["guard"]["fired"] == 0
      and abs(_slow["guard"]["standoff"] - _slow["partition"]["standoff"]) < 1e-9,
      "(85 deg over 3 s: area never below %.2fx, both reach %.3f m)"
      % (_slow["partition"]["min_area"], _slow["guard"]["standoff"]))

_edge = {(d, s): _act("guard", d, s, T=4.0 + 2 * s)["fired"] > 0
         for d, s in ((75, 1.0), (75, 1.5), (80, 1.5), (80, 2.0),
                      (85, 2.0), (85, 3.0))}
check("the window is a race between the fold and the depth loop",
      (_edge[(75, 1.0)] and not _edge[(75, 1.5)]
       and _edge[(80, 1.5)] and not _edge[(80, 2.0)]
       and _edge[(85, 2.0)] and not _edge[(85, 3.0)]),
